---
title: "Découverte de la SciViews Box 2018"
...
- Citez une application permettant d'écrire des équations mathématiques^[Vous devez allez dans le menu `Applications` en haut à gauche dans la barre système puis recherchez `EqualX` soit dans les favoris, soit dans la section `Office`.].
    `r iframe("equalx.gif", 1026, 768)`
...
- Trouvez une application permettant de choisir des charactères spéciaux^[Vous devez aller dans le menu système `Applications` puis dans la section `Accessoires`, pour sélectionner `Characters` ou `Character Map`.].
    `r iframe("characters.gif", 1026, 768)`

______

- Trouvez une application permettant de visualiser l'occupation de l'espace sur le disque^[Dans le menu système `Applications` et dans la section `System`, vous sélectionnez `Disk Usage Analyzer`.].
    `r iframe("disk.gif", 1026, 768)`
...
- Dans l'analyseur de disque, entrez dans le disque principal de la machine virtuel `box2018a` (l'application calcule l'occupation des fichiers qu'il contient... soyez patient, cela prend un peu de temps).
    + Quel dossier occupe le plus de place sur le disque ?^[Le dossier `usr` est le plus volumnieux.]
    + Au sein de ce dossier quel sous-dossier occupe le plus de place ?^[Le sous-dossier `local` est le plus gros au sein de `/usr`.]
    + Au sein de ce second dossier quel sous-dossier occupe le plus de place ?^[Le sous-dossier `lib` occupe le plus de place dans `/usr/local`.] 
...
- Recherchez votre fichier `test.txt` dans votre ordinateur hôte (Windows ou MacOS, par exemple) et notez la position du fichier^[Le fichier n'est pas accessible depuis la machine hôte.].

- Déplacez maintenant ce fichier `test.txt` vers le bureau de la machine virtuelle^[Ce fichier n'est toujours pas accessible depuis la machine hôte.]. 

- Déplacez à nouveau ce fichier dans le sous-dossier `projects` du dossier `shared`. Qu'observez-vous à partir de la machine hôte ?^[Le fichier `test.txt` est à présent visible et manipulable également depuis la machine hôte. Il s'y trouve bien dans le sous-dossier `projects` du dossier `shared`.]
